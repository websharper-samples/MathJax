...
{
 "use strict";
 var Global,Samples,TeXInput,WebSharper,Obj,UI,Var$1,Var,Operators,Templating,Runtime,Server,TemplateInstance,Arrays,ProviderBuilder,Handler,MathJax_Templates,View,ConcreteVar,Snap,Client,Templates,Doc,JavaScript,Pervasives,Collections,List,Unchecked,System,Guid,Dictionary,HashSet,Client$1,Seq,AttrModule,Abbrev,Fresh,EventTarget,Node,JS,DomUtility,Object,DictionaryUtil,Enumerator,T,AttrProxy,CheckedInput,Numeric,Attrs,SC$1,WindowOrWorkerGlobalScope,SC$2,Docs,DynamicAttrNode,SC$3,Prepare,Slice,KeyCollection,DocElemNode,CharacterData,An,Settings,Mailbox,Elt,Array,HashSetUtil,Strings,Attrs$1,Dyn,Docs$1,RunState,NodeSet,Anims,SC$4,Updates,List$1,T$1,SC$5,Concurrency,AppendList,SC$6,Queue,Easing,AsyncBody,SC$7,CT,HashSet$1,String,Scheduler,CancellationTokenSource,DomNodes,Char,Error,OperationCanceledException,Lazy,SC$8,LazyExtensionsProxy,LazyRecord,MathJax,Hub,IntelliFactory,Runtime$1,$,console,Math,Date;
 Global=self;
 Samples=Global.Samples=Global.Samples||{};
 TeXInput=Samples.TeXInput=Samples.TeXInput||{};
 ...
 MathJax_Templates=Global.MathJax_Templates=Global.MathJax_Templates||{};
 ...
 Math=Global.Math;
 Date=Global.Date;
 TeXInput.Main=function()
 {
  var r,r$1,rvExpression,viewExpression,tr,a,b,p,i;
  function a$1(e,v)
  {
   var _this;
   _this=Global.jQuery("#tex div").empty();
   _this.append.apply(_this,["$$"+v+"$$"]);
  ...
  rvExpression=Var$1.Create$1("\\frac{2\\cdot x\\cdot\\left({ x}^{9}+{ x}^{2}\\right)-{ x}^{2}\\cdot\\left(9\\cdot{ x}^{8}+2\\cdot x\\right)}{{\\left({ x}^{9}+{ x}^{2}\\right)}^{2}}+2\\cdot{ x}^{3}");
  viewExpression=rvExpression.get_View();
  tr=Doc.Element("div",[AttrModule.DynamicCustom(function($1)
  {
   return function($2)
   {
    return a$1($1,$2);
   };
  },viewExpression)],[Doc.TextView(View.Map(function(x)
  {
   return"$$"+x+"$$";
  },viewExpression))]);
  Templates.LoadLocalTemplates("");
  Doc.RunById("tex",tr);
  a=(b=ProviderBuilder.Make().WithHole({
   $:8,
   $0:"expression",
   $1:rvExpression
  }),(p=Handler.CompleteHoles(b.k,b.h,[["expression",0]]),(i=new TemplateInstance.New(p[1],MathJax_Templates.main(p[0])),(b.i=i,i)))).get_Doc();
  Templates.LoadLocalTemplates("");
  Doc.RunById("main",a);
 };
 ...
 MathJax_Templates.main=function(h)
 {
  return h?Templates.GetOrLoadTemplate("index",{
   $:1,
   $0:"main"
  },function()
  {
  ...
  },h):Templates.PrepareTemplate("index",{
   $:1,
   $0:"main"
  },function()
  {
